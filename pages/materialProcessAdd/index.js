...
Page({
  ...
  handle_long(e) {
    ...
    console.log(this.data.color_no);
    ...
    this.setData({
      ...
      timer_out: e.currentTarget.dataset.item.timer_out,
    });
    this.data.color_data.map((v, i) => {
      if (this.data.timer_out == v.timer_out) {
        console.log(v);
        if (v.color_no != "无编号") {
          v.color_no = "";
        }
  ...
  affirm_color() {
    ...
    if (
      ...
      if (this.data.timer_out != "") {
        this.data.color_data.map((v, i) => {
          if (v.timer_out == this.data.timer_out) {
    ...
    this.wecropper.getCropperImage((src) => {
      if (src) {
        ...
        if (this.data.we_cropper_active_status == 1) {
          ...
          //     });
          //     if (data.error_code === 0) {
          //       that.pregMaterial(data.data.words_result, material.data.data);
          //       that.pregMaterialsno(data.data.words_result);
          //       that.preg_breadth_price(data.data.words_result);
          //       that.preg_grammage_price(data.data.words_result);
          //       that.setData({
          //         words_result: data.data.words_result,
          //         tempFilePaths: src,
          //         we_cropper_active: false,
          //       });
...
instock: "0"
material_data: [{…}]
materials_class_id: "7"
materials_class_name: "针织"
materials_mainclass_id: "3"
materials_mainclass_name: "主料"
...
/*
    ...
    obj["materials_supplier_id"] = this.data.materials_supplier_id;
    obj["materials_class_id"] = this.data.materials_class_id;
    obj["unit_name"] = this.data.unit_name;
    obj["materialsname"] = this.data.name;
    obj["wsale_price"] = this.data.unit_price;
    obj["instock"] = Number(this.data.radio);
    obj["arrival_time"] = this.data.timer_price;
    obj["remarks"] = this.data.remark_price;
